...
appointment.cancelled / appointment.rescheduled:
  Slot liberado → ENFILEIRA notify_waitlist_slot_available para os escopos
  SERVICE (cada service_id do appointment) e PROFESSIONAL (S2.1-A).
  O envio httpx roda no worker, não no request de cancel/reschedule.
...
def handle_appointment_cancelled_waitlist(event) -> None:
    ...
    O EventBus permanece síncrono — torná-lo assíncrono afeta todos os eventos
    de domínio, inclusive os financeiros, e é decisão pendente. O corte é aqui,
    dentro do handler.
    """
...
def notify_waitlist_slot_available(self, company_id: str, scope_type: str, target_id: str, reason: str):
    """Notifica o 1º elegível da fila para um escopo — fora do request (S2.1-A).

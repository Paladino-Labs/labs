...
Contrato:
  - send_booking_confirmation/send_reschedule_confirmation apenas ENFILEIRAM.
    ...
    roda no worker, FORA do request HTTP. Antes do S2.1-A era "fire-and-forget"
...
def send_booking_confirmation(
    ...
    """Enfileira a confirmação de agendamento ao cliente (S2.1-A).
...
def send_reschedule_confirmation(
    ...
    """Enfileira a confirmação de reagendamento ao cliente (S2.1-A)."""
    _enqueue_appointment_communication(
...
def _enqueue_appointment_communication(
    ...
    importlib.import_module resolve a task no momento da chamada — o patch de
    `app.workers.communication_worker.send_appointment_communication` alcança
    este caminho, e não há import de worker no topo do módulo.

    O payload é 100% escalar: IDs + o token CRU (o banco guarda só o hash,
    irreversível, então o token precisa viajar aqui). Broker indisponível é
    logado e engolido — o enfileiramento é best-effort; a resiliência de envio
    é da própria task.
    """
    try:
        import importlib
